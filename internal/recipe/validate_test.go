...
func TestValidateAdditionalStructuralRules(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "node tooling without fields", mutate: func(m *Manifest) { m.Setup.NodeTooling = true }, want: `setup.nodeTooling: requires a "linting" field`},
		{name: "expo styling without fields", mutate: func(m *Manifest) { m.Setup.ExpoStyling = true }, want: `setup.expoStyling: requires a "styling" field`},
		{name: "node project dir without tooling", mutate: func(m *Manifest) { m.Setup.NodeProjectDir = "frontend" }, want: "setup.nodeProjectDir"},
		{name: "node project dir traversal", mutate: func(m *Manifest) { m.Setup.NodeTooling = true; m.Setup.NodeProjectDir = "../frontend" }, want: "setup.nodeProjectDir"},
